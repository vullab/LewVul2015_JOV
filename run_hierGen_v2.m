function [samps rmse]=run_hierGen_v2(firstTarg,clusAssign,numSamps,objNoise,fname,clusSd)
%% modFit2_hierGen
...
if ~exist(fname)
    ...
    for ei=1:numEnv
        if ei<31
            encNoise=mean(objNoise(1:30));
        else
            encNoise=mean(objNoise(31:70));
        end
        
        for si=1:numSamps
            
            samps{ei,si}=takeSamp(firstTarg{ei},clusAssign{ei},encNoise,clusSd{ei});
    ...
    function estPos=takeSamp(objLocs,clusAssign,objNoise,clusSd)
        ...
        for ci=1:numC
            ...
            clusNoise(ci)=clusSd(ci)/sqrt(numIn(ci));
